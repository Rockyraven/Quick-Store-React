...
const WishProvider = ({children}) => {
    ...
    const addToWishList = (product) => {
        ...
        if(isItemExit){
            setWishList(prev => prev.map(item => item._id === isItemExit._id ? {...item,quantity: item.quantity+1} : item) )
        }
        else{
...
const addToCart = (product)=> {
   ...
   if(isItemExit){
       setCart(prev => prev.map(item => item._id === isItemExit._id ? {...item,quantity: item.quantity+1} : item) )
   }
   else{
...
const increaseQuantity = (_id) => {
    setCart(prev => prev.map(item => item._id === _id ? {...item,quantity: item.quantity+1} : item) )

}
const decreaseQuantity = (_id) => {

    setCart(prev => prev.map(item => item._id === _id ? item.quantity >1 ? {...item,quantity: item.quantity-1} : {...item,quantity: 1} : item) )
    ...
    return(
        <wishlistContext.Provider value={{ wishlist, setWishList, addToWishList, removeFromWishList, addToCart, cart, setCart, removeToCart, increaseQuantity, decreaseQuantity, total }}>
        {children}
